...
const startBot = (bot) => {
	try {
		...
		bot.on('photo', async (msg) => {
			bot.sendMessage(msg.chat.id, 'Got your image! Wait a few second, we\'ll send your avatar');
			...
			await Promise.all([
				bot.sendPhoto(msg.chat.id, buffer, {}, {filename: 'ua-avatar'}),
				bot.sendDocument(msg.chat.id, buffer, {}, {filename: 'ua-avatar'}),
			])

			bot.sendMessage(msg.chat.id, '🇺🇦 *Here you are! Set it to your social media.*\n\nAlso, you can support Ukraine in this hard times. What can you do?\n\n1. [Donate to Ukraine Army](https://uahelp.monobank.ua/)\n2. [Help to disable Russian services with DDoS](https://help-ukraine-win.com/)\n\n*Glory to Ukraine*', {parse_mode: 'Markdown', disable_web_page_preview: true})
		})

	} catch (e) {
		console.log(e);
		bot.sendMessage('177498086', e);
		startBot();
	}
}
